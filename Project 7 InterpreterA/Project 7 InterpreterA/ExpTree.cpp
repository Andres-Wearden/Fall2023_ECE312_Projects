...
Node* ExpTree::add(Node* root, vector<Node*>& expr) {
    ...
    if(first->optr == "~" || first->optr == "!" ){
        ...
        root->right = nullptr;
    }else if (!(first->isOperand)) { // if first is an operator, add 2 operands
        root->left = add(root->left, expr);
...
int ExpTree::evaluate(Node* root){
    if(root->optr == "~"){
        return evaluate(root->left)*-1;
    }else if(root->optr == "!"){
        return (evaluate(root->left) == 0);
    }
    if(root->isOperand){
        return root->operand;
    }
    int leftNum = evaluate(root->left);
    int rightNum = evaluate(root->right);
    if(root->optr == "+"){
...
int ExpTree::evaluate(){
    return evaluate(this->overallRoot);
}
