 ...
 */
int canTurnLeft(int row, int col);
int canTurnRight(int row, int col);
int canTurnUp(int row, int col);
int canTurnDown(int row, int col);
...
int minRec2(int x[], int n) {
    ...
    if(n == 1){
        ...
        minLHS = minRec2(x, n/2);
        minRHS = minRec2(x + (n/2), n - (n/2));
        if(minLHS < minRHS){
            return minLHS;
        }else{
            return minRHS;
...
double sqrtRec(double x, double low_guess, double high_guess) {
    double midpoint = (high_guess + low_guess)/2.0;
    double midpointSquare = midpoint*midpoint;
    double EPSILON = 1e-15;
    if((high_guess - low_guess) < EPSILON){
        return midpoint;
    }else{
        if(midpointSquare < x){
            return sqrtRec(x, midpoint, high_guess);
        }else{
            return sqrtRec(x,low_guess, midpoint);
        }
    }
}
...
int strCompare(char* str1, char* str2) {
    int asciiStr1 = (int)(unsigned char)*str1;
    int asciiStr2 = (int)(unsigned char)*str2;
...
int strCompare2(char* str1, char* str2) {
    int asciiStr1 = whatLetterNull(*str1);
    int asciiStr2 = whatLetterNull(*str2);
    if(asciiStr1 == -1){
 ...
 */
//int solveMazeRec2(int row, int col){
//    if(row >= MATRIX_SIZE){
//        return 1;
//    }
//    maze[row][col] = 2;
//    if(canTurnDown(row, col) == 1){
//        if(solveMazeRec(row + 1, col) == 0){
//            return solveMazeRec(row, col);
//        }
//    }
//    if(canTurnUp(row, col) == 1){
//        if(solveMazeRec(row - 1, col) == 0){
//            return solveMazeRec(row, col);
//        }
//    }
//    if(canTurnLeft(row, col) == 1){
//        if(solveMazeRec(row, col - 1) == 0){
//            return solveMazeRec(row, col);
//        }
//    }
//    if(canTurnRight(row, col) == 1){
//        if(solveMazeRec(row, col + 1) == 0){
//            return solveMazeRec(row, col);
//        }
//    }
...
int solveMazeRec(int row, int col){
    ...
    if((row > MATRIX_SIZE - 1) || (row < 0) || (col > MATRIX_SIZE - 1) || (col < 0)){
        return 0;
    }
    maze[row][col] = 2;
    if(canTurnDown(row, col) == 1) {
...
void solveMazeIt(int row, int col) {
    ...
    while (row < MATRIX_SIZE - 1) {
        return;// the exit is the only open square
    }
}
